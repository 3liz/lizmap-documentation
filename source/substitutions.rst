.. |qgis_2| replace:: **QGIS 2**
.. |qgis_3| replace:: **QGIS 3**

.. |lizmap_2_8| replace:: **Lizmap 2.8**
.. |lizmap_3_0| replace:: **Lizmap 3.0**
.. |lizmap_3_1| replace:: **Lizmap 3.1**
.. |lizmap_3_2| replace:: **Lizmap 3.2**
.. |lizmap_3_3| replace:: **Lizmap 3.3**
.. |lizmap_3_4| replace:: **Lizmap 3.4**

.. |add_layer| replace:: Click the |add_layer_svg| button.
.. |edit_layer| replace:: To edit a layer in the table, select it and click the |edit_layer_svg| button.
.. |remove_layer| replace:: To remove a layer from the table, select it and click the |remove_layer_svg| button.
.. |move_up_down_layer| replace:: To move a layer up or down, select it and click |down_layer_svg| or |up_layer_svg| buttons. The order will change in Lizmap too.
.. |wfs_layer| replace:: The layer must be published as WFS. See :ref:`publish_layer_wfs`.
.. |field_alias| replace:: Fields will be displayed with their alias if it's defined. See :ref:`alias_on_fields`.

.. |add_layer_svg| image:: /images/icons/symbologyAdd.svg
.. |edit_layer_svg| image:: /images/icons/symbologyEdit.svg
.. |remove_layer_svg| image:: /images/icons/symbologyRemove.svg
.. |down_layer_svg| image:: /images/icons/mActionArrowDown.svg
.. |up_layer_svg| image:: /images/icons/mActionArrowUp.svg
